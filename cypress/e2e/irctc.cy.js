...
let isCaptchaFeederOn = Cypress.env('CAPTCHA_FEEDER');
...
describe('IRCTC TATKAL BOOKING', () => {
  it('Tatkal Booking Begins......', () => {
    ...
    const runBookingProcess = () => {
      ...
      const UPI_ID = Cypress.env().UPI_ID ? Cypress.env().UPI_ID
          : UPI_ID_CONFIG;
      ...
      cy.get('input[placeholder="User Name"]').invoke('val', username).trigger(
          'input')
      cy.get('input[placeholder="Password"]').invoke('val', password).trigger(
          'input')

      // Submitting captcha block starts........
      cy.submitCaptcha().then(() => {
        if(isCaptchaFeederOn) {
          //This when on will feed the captcha dataset to fine tune the model
          runBookingProcess();
        }
        ...
        cy.get('body').then((el) => {
          if (el[0].innerText.includes('Your Last Transaction')) {
            cy.get(
                '.ui-dialog-footer > .ng-tns-c19-3 > .text-center > .btn').click()
          }
        })

        // from station
        cy.get('.ui-autocomplete > .ng-tns-c57-8').should('be.visible').type(
            SOURCE_STATION, {delay: 600})
        // cy.wait(600)
        cy.get('#p-highlighted-option').should('be.visible').click()

        // to station
        cy.get('.ui-autocomplete > .ng-tns-c57-9').should('be.visible').type(
            DESTINATION_STATION, {delay: 600})
        // cy.wait(600)
        cy.get('#p-highlighted-option').should('be.visible').click()
        ...
        cy.get(':nth-child(n) > .bull-back').each((div, index) => {
          ...
          if (div[0].innerText.includes(TRAIN_NO) && div[0].innerText.includes(
              TRAIN_COACH)) {

            cy.bookUntilTatkalGetsOpen(div, TRAIN_COACH, TRAVEL_DATE, TRAIN_NO,
                TATKAL).then(() => {
            ...
            cy.get('input[formcontrolname="passengerAge"]').each(
                (inputDiv, index) => {
                  ...
                  cy.wrap(inputDiv).invoke('val', PASSENGER['AGE']).trigger(
                      'input')
            ...
            cy.get('body').then((body) => {
              if (body.find(
                  'select[formcontrolname="passengerFoodChoice"]').length > 0) {
            ...
            cy.solveCaptcha().then(() => {
              ...
              cy.get(
                  '.col-sm-9 > app-bank > #bank-type > :nth-child(2) > table > tr > :nth-child(1) > .col-lg-12 > .border-all > .col-xs-12 > .col-pad').click()
              ...
              cy.wait("@payment", {timeout: 200000}).then((interception) => {
                ...
                if (UPI_ID && isValidUpiId) {
                  ...
                  cy.get(
                      '.brdr-box > :nth-child(2) > ._1WLd > :nth-child(1) > .xs-hover-box > ._Mzth > .form-ctrl').type(
                      UPI_ID)
